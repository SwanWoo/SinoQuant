...
# 初始化SinaQuant日志系统
from sinoquant.utils.logging_init import init_logging
init_logging()
...
class AnalysisService:
    ...
    def _get_trading_graph(self, config: Dict[str, Any]) -> SinaQuantGraph:
        """获取或创建SinaQuant图实例（带缓存）- 与单股分析保持一致"""
        config_key = json.dumps(config, sort_keys=True)

        if config_key not in self._trading_graph_cache:
            # 直接使用完整配置，不再合并DEFAULT_CONFIG（因为create_analysis_config已经处理了）
            # 这与单股分析服务和web目录的方式一致
            self._trading_graph_cache[config_key] = SinaQuantGraph(
                selected_analysts=config.get("selected_analysts", ["market", "fundamentals"]),
                debug=config.get("debug", False),
                config=config
            )

            logger.info(f"创建新的SinaQuant实例: {config.get('llm_provider', 'default')}")
    ...
    def _execute_analysis_sync_with_progress(self, task: AnalysisTask, progress_tracker: RedisProgressTracker) -> AnalysisResult:
        ...
        try:
            ...
            # 如果模型未配置，提前报错而非静默使用空值
            if not quick_model or not deep_model:
                raise ValueError(
                    f"LLM 模型未配置（quick={quick_model}, deep={deep_model}）。"
                    f"请先在前端「设置 → LLM 配置」页面配置模型后再执行分析。"
                )
            ...
            # 根据模型名称动态查找供应商（同步版本）
            llm_provider = unified_config.get_system_settings().get("llm_provider", "")
    ...
    async def submit_single_analysis(
        ...
        try:
            ...
            if not getattr(params, 'quick_analysis_model', None):
                params.quick_analysis_model = effective_settings.get("quick_analysis_model") or None
            if not getattr(params, 'deep_analysis_model', None):
                params.deep_analysis_model = effective_settings.get("deep_analysis_model") or None
