...
class ModelCapabilityService:
            ...
            if doc and "llm_configs" in doc:
                ...
                for config_dict in llm_configs:
                    if config_dict.get("model_name") == model_name:
                        ...
                        provider_name = (config_dict.get("provider") or "").lower()
                        ...
                        # DeepSeek 在当前实现中支持工具调用，旧配置缺少该特性时自动补齐
                        if provider_name == "deepseek" and ModelFeature.TOOL_CALLING not in features_enum:
                            features_enum.append(ModelFeature.TOOL_CALLING)
                            logger.info(f"🔧 [模型能力修正] 为 {model_name} 自动补充 TOOL_CALLING 特性")
    ...
    def _get_default_models(self) -> Tuple[Optional[str], Optional[str]]:
        """获取默认模型对 — 未配置时返回 (None, None)"""
        try:
