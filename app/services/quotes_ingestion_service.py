...
class QuotesIngestionService:
    ...
    async def run_once(self) -> None:
        ...
        try:
            ...
            # 按优先级依次尝试所有数据源，直到有一个成功
            quotes_map = None
            source_name = None
            tried_sources = []

            for attempt in range(len(self._rotation_sources)):
                source_type, akshare_api = self._get_next_source()
                tried_sources.append(source_type or akshare_api)
            ...
            if not quotes_map:
                logger.error(f"⚠️ 所有数据源均失败 (已尝试: {tried_sources})，跳过本次入库")
                # 记录失败状态
                await self._record_sync_status(
                    success=False,
                    source=",".join(tried_sources),
                    records_count=0,
            ...
            # 指数行情入库
            await self._fetch_and_store_index_quotes(trade_date)
    ...
    async def _fetch_and_store_index_quotes(self, trade_date: str) -> None:
        """获取指数实时行情并入库到 market_quotes（type=index 区分）"""
        try:
            from app.services.data_sources.akshare_adapter import AKShareAdapter
            adapter = AKShareAdapter()
            if not adapter.is_available():
            ...
            idx_quotes = adapter.get_index_spot_quotes()
            if not idx_quotes:
                return
            ...
            for ak_code, q in idx_quotes.items():
            ...
            if ops:
                ...
                logger.info(f"指数行情入库: upserted={result.upserted_count}, modified={result.modified_count}")
        except Exception as e:
            logger.warning(f"指数行情入库失败: {e}")
