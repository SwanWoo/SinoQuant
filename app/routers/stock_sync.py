    ...
    try:
        ...
        if request.sync_realtime:
            try:
                ...
                # 🔥 如果同步失败，添加错误详情
                if not success:
                    errors = realtime_result.get("errors", [])
                    if errors:
                        error_msg = errors[0].get("error", "获取行情失败")
                        realtime_sync_result["error"] = error_msg
                    else:
                        # 🔥 检查是否在交易时间
                        from app.utils.trading_time import is_trading_time
                        if not is_trading_time():
                            realtime_sync_result["error"] = "当前不在交易时间（交易日 9:30-11:30, 13:00-15:00），无法获取实时行情"
                        else:
                            realtime_sync_result["error"] = "获取行情失败，请检查股票代码是否正确"

                result["realtime_sync"] = realtime_sync_result
    ...
    try:
        ...
        if request.sync_basic:
            try:
                ...
                if request.data_source == "tushare":
                    from sinoquant.dataflows.providers.china.tushare import TushareProvider
