...
async def _execute_market_news_sync(sync_service, request: NewsSyncRequest):
    """执行市场新闻同步 — 使用 AKShare 东方财富全球快讯"""
    try:
        logger.info("📰 开始后台市场新闻同步（AKShare 东方财富全球快讯）...")
        ...
        news_items = await provider.get_stock_news(
            symbol=None,
            limit=request.max_news_per_source
        )

        if news_items:
            saved = await news_service.save_news_data(
                news_items, "market_news", "CN"
            )
            logger.info(f"✅ 市场新闻同步完成: 获取 {len(news_items)} 条, 保存 {saved} 条")
        else:
            logger.warning("⚠️ 未获取到市场新闻")

    except Exception as e:
        logger.error(f"❌ 后台市场新闻同步失败: {e}", exc_info=True)
