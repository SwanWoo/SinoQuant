...
def _sanitize_report_doc(doc: Dict[str, Any]) -> Dict[str, Any]:
    """对报告文档做统一清洗，防止旧数据透传内部 history/思维链内容。"""
    safe_doc = dict(doc or {})
    safe_doc["summary"] = remove_thinking_content(str(safe_doc.get("summary", "") or "")).strip()
    safe_doc["recommendation"] = remove_thinking_content(
        str(safe_doc.get("recommendation", "") or "")
    ).strip()
    safe_doc["reports"] = sanitize_report_modules(safe_doc.get("reports", {}))
    return safe_doc
...
async def get_reports_list(
    ...
    market_filter: Optional[str] = Query(None, description="市场筛选（A股）"),
    ...
    try:
        ...
        async for doc in cursor:
            ...
            report = {
                ...
                "summary": remove_thinking_content(str(doc.get("summary", "") or "")).strip(),
    ...
    try:
        ...
        if not doc:
            ...
            report = {
                ...
                "summary": remove_thinking_content(str(r.get("summary", "") or "")).strip(),
                "reports": sanitize_report_modules(r.get("reports", {})),
                "source": "analysis_tasks",
                "task_id": tasks_doc.get("task_id", report_id),
                "recommendation": remove_thinking_content(
            ...
            report = {
                ...
                "research_depth": doc.get("research_depth", 1),
                "summary": remove_thinking_content(str(doc.get("summary", "") or "")).strip(),
                "reports": sanitize_report_modules(doc.get("reports", {})),
                "source": doc.get("source", "unknown"),
                "task_id": doc.get("task_id", ""),
                "recommendation": remove_thinking_content(
                    str(doc.get("recommendation", "") or "")
                ).strip(),
...
async def get_report_module_content(
    ...
    try:
        ...
        content = normalize_report_content(reports[module], module)
...
async def download_report(
    ...
    try:
        ...
        safe_doc = _sanitize_report_doc(doc)
        stock_symbol = safe_doc.get("stock_symbol", "unknown")
        analysis_date = safe_doc.get("analysis_date", datetime.now().strftime("%Y-%m-%d"))

        if format == "json":
            # JSON格式下载
            content = json.dumps(safe_doc, ensure_ascii=False, indent=2, default=str)
            ...
            # Markdown格式下载
            reports = safe_doc.get("reports", {})
            content_parts = []

            # 添加标题
            content_parts.append(f"# {stock_symbol} 分析报告")
            content_parts.append(f"**分析日期**: {analysis_date}")
            content_parts.append(f"**分析师**: {', '.join(safe_doc.get('analysts', []))}")
            content_parts.append(f"**研究深度**: {safe_doc.get('research_depth', 1)}")
            content_parts.append("")

            # 添加摘要
            if safe_doc.get("summary"):
                content_parts.append("## 执行摘要")
                content_parts.append(safe_doc["summary"])
                content_parts.append("")
            ...
            try:
                # 生成 Word 文档
                docx_content = report_exporter.generate_docx_report(safe_doc)
            ...
            try:
                # 生成 PDF 文档
                pdf_content = report_exporter.generate_pdf_report(safe_doc)
