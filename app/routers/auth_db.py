...
async def login(payload: LoginRequest, request: Request):
    ...
    try:
        ...
        refresh_token = AuthService.create_access_token(sub=user.username, expires_delta=60*60*24*settings.REFRESH_TOKEN_EXPIRE_DAYS)
...
async def register(payload: RegisterRequest, request: Request):
    ...
    logger.info(f"📝 注册请求 - 用户名: {payload.username}, 邮箱: {payload.email}")
    
    try:
        # 验证输入
        if not payload.username or not payload.email or not payload.password:
            logger.warning("❌ 注册失败 - 必填字段为空")
        ...
        if not new_user:
            logger.warning(f"❌ 注册失败 - 用户名或邮箱已存在: {payload.username}")
            await log_operation(
                user_id="unknown",
                username=payload.username,
                action_type=ActionType.USER_LOGIN,
                action="用户注册",
                details={"reason": "用户名或邮箱已存在"},
                success=False,
        ...
        refresh_token = AuthService.create_access_token(sub=new_user.username, expires_delta=60*60*24*settings.REFRESH_TOKEN_EXPIRE_DAYS)
...
async def refresh_token(payload: RefreshTokenRequest):
    ...
    try:
        ...
        new_refresh_token = AuthService.create_access_token(sub=token_data.sub, expires_delta=60*60*24*settings.REFRESH_TOKEN_EXPIRE_DAYS)
