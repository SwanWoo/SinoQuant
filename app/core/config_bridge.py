"""
配置桥接模块
将统一配置系统的配置桥接到环境变量，供 SinaQuant 核心库使用
"""
...
def bridge_config_to_env():
    ...
    这样 SinaQuant 核心库就能通过环境变量读取到用户配置的数据
    """
    try:
        ...
        # 桥接 MongoDB 数据库名称
        mongodb_db_name = os.getenv("MONGODB_DATABASE_NAME", "sinoquant")
        os.environ["MONGODB_DATABASE_NAME"] = mongodb_db_name
        logger.info(f"  ✓ 桥接 MONGODB_DATABASE_NAME: {mongodb_db_name}")
        bridged_count += 1

        # 1. 桥接大模型配置（基础 API 密钥）
        # 🔧 [仅数据库] API Key 只从数据库读取，不从 .env 环境变量读取
        # 🔥 数据源配置（Tushare、FinnHub）和基础设施配置仍从 .env 读取
        try:
            ...
            for provider in providers:
                ...
                # 🔒 LLM API Key 只从数据库获取，清除环境变量中可能残留的 .env 值
                ...
                if provider.api_key and not provider.api_key.startswith("your_"):
                    ...
                    logger.debug(f"  ⏭️  {env_key} 数据库中未配置有效的 API Key")
        ...
        try:
            ...
            if use_mongodb.lower() == "true":
                try:
                    ...
                    if config_manager.mongodb_storage.is_connected():
                        logger.info("✅ sinoquant MongoDB 存储已启用")
                    else:
                        logger.warning("⚠️ sinoquant MongoDB 连接失败，将使用 JSON 文件存储")
            ...
            logger.error(f"❌ 重新初始化 sinoquant MongoDB 存储失败: {e}")
            import traceback
            logger.error(traceback.format_exc())

        # 7. 同步定价配置到 sinoquant 的 config/pricing.json
        ...
        logger.warning("⚠️  SinaQuant 将使用 .env 文件中的配置")
        return False
...
def _sync_pricing_config(llm_configs):
    """
    同步定价配置到 sinoquant 的 config/pricing.json
