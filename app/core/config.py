"""
应用配置模块（Pydantic Settings）

职责：集中管理所有运行时配置，支持从 .env 文件和环境变量读取。

配置加载优先级：环境变量 > .env 文件 > 代码中的默认值
（由 pydantic-settings 的 SettingsConfigDict 自动处理）

配置分组：
  - 基础配置：DEBUG / HOST / PORT / CORS 白名单
  - MongoDB：连接地址、认证、连接池、超时参数
  - Redis：连接地址、认证、连接池
  - JWT：密钥、算法、过期时间
  - 队列：任务队列大小、重试次数、心跳间隔
  - 并发控制：用户级/全局并发限制、每日配额
  - 代理：HTTP_PROXY / HTTPS_PROXY / NO_PROXY（国内数据源绕过代理）
  - Tushare / AKShare / BaoStock：各数据源的同步任务配置（CRON 表达式）
  - 实时行情：采集间隔、接口轮换、Tushare 权限自动检测
  - 新闻同步：采集频率、单次数量限制

使用方式：
  from app.core.config import settings
  db_name = settings.MONGODB_DATABASE
  mongo_uri = settings.MONGO_URI  # 自动根据用户名密码拼接

注意：
  - 所有配置项都有合理的默认值，开发环境零配置即可启动
  - 修改 .env 后需重启服务（pydantic-settings 不支持热加载）
  - CONFIG_SOT 控制配置真相来源：file（推荐）/ db / hybrid
"""
...
# 旧版环境变量别名（已废弃）：将 API_HOST/API_PORT/API_DEBUG 映射为 HOST/PORT/DEBUG
# 保留此逻辑是为了向后兼容旧版 .env 文件
_LEGACY_ENV_ALIASES = {
    "API_HOST": "HOST",
    "API_PORT": "PORT",
    "API_DEBUG": "DEBUG",
...
class Settings(BaseSettings):
    ...
    所有字段通过 pydantic Field 定义，支持：
      - 默认值：每个字段都有合理的默认值
      - 环境变量覆盖：同名字段自动从 .env 和 os.environ 读取
      - 类型校验：启动时自动校验类型和范围（如 ge=1, le=3650）
      - description：字段的中文说明，便于 /docs 端点展示
    """
    # 基础配置
    ...
    MONGODB_DATABASE: str = Field(default="sinoquant")
    ...
    GLOBAL_CONCURRENT_LIMIT: int = Field(default=20)
    ...
    LOG_FILE: str = Field(default="logs/sinoquant.log")
    ...
    ENCRYPTION_KEY: str = Field(
        default="",
        description="对称加密密钥（Fernet格式），用于加密存储 API Key 等敏感凭证。生产环境必须配置！"
    )
...
# 将代理配置写入环境变量，使 requests/httpx 等库能自动读取
# 这是因为 Python 的 requests 库通过 os.environ['HTTP_PROXY'] 等变量识别代理
