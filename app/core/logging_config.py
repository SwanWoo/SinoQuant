...
def setup_logging(log_level: str = "INFO"):
    ...
    try:
        ...
        if cfg_path.exists() and toml_loader is not None:
            ...
            # 主日志文件（sinoquant.log）
            main_log = main_handler_cfg.get("filename", str(Path(file_dir) / "sinoquant.log"))
            ...
            # 测试主日志文件是否可写
            if main_enabled:
                test_logger = logging.getLogger("sinoquant")
                test_logger.info(f"🔍 测试主日志文件写入: {main_log}")
                print(f"🔍 [setup_logging] 已向 sinoquant logger 写入测试日志")
