"""
统一配置管理系统
整合 config/、sinoquant/config/ 和 webapi 的配置管理
"""
...
class ConfigPaths:
    ...
    sinoquant_config_dir: Path = Path("sinoquant/config")
...
class UnifiedConfigManager:
    ...
    def get_default_model(self) -> Optional[str]:
        """获取默认模型（向后兼容）— 未配置时返回 None"""
        settings = self.get_system_settings()
        # 优先返回快速分析模型，保持向后兼容
        return settings.get("quick_analysis_model") or settings.get("default_model") or None
    ...
    def get_quick_analysis_model(self) -> Optional[str]:
        """获取快速分析模型 — 未配置时返回 None"""
        settings = self.get_system_settings()
        # 优先读取新字段名，如果不存在则读取旧字段名（向后兼容）
        return settings.get("quick_analysis_model") or settings.get("quick_think_llm") or None

    def get_deep_analysis_model(self) -> Optional[str]:
        """获取深度分析模型 — 未配置时返回 None"""
        settings = self.get_system_settings()
        # 优先读取新字段名，如果不存在则读取旧字段名（向后兼容）
        return settings.get("deep_analysis_model") or settings.get("deep_think_llm") or None
    ...
    def get_database_configs(self) -> List[DatabaseConfig]:
        ...
        mongodb_config = DatabaseConfig(
            ...
            database=os.getenv("MONGODB_DATABASE", "sinoquant"),
            enabled=True,
