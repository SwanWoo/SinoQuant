...
对应 MongoDB 集合: stock_basic_info, market_quotes
...
# 枚举类型定义
MarketType = Literal["CN"]  # 市场类型: 目前仅支持中国A股
ExchangeType = Literal["SZSE", "SSE", "BSE"]  # 交易所: 深交所/上交所/北交所
StockStatus = Literal["L", "D", "P"]  # 上市状态: L-上市 D-退市 P-暂停
CurrencyType = Literal["CNY"]  # 货币类型


class MarketInfo(BaseModel):
    """市场信息 — 交易所/货币/时区/交易时间等"""
...
class StockBasicInfoExtended(BaseModel):
    """股票基础信息扩展模型 — 对应 stock_basic_info 集合，含代码/行业/财务/市值等"""
    # === 标准化字段 (主要字段) ===
...
class MarketQuotesExtended(BaseModel):
    """实时行情扩展模型 — 对应 market_quotes 集合，含价格/涨跌/五档/成交量等"""
    # === 标准化字段 (主要字段) ===
    symbol: str = Field(..., description="6位股票代码", pattern=r"^\d{6}$")
