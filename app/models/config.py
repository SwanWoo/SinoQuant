...
对应 MongoDB 集合: llm_providers, model_catalog, system_configs, token_usage
"""
...
class ModelProvider(str, Enum):
    """大模型提供商枚举 — 支持直连厂商和聚合渠道(302.AI/OpenRouter等)"""
    OPENAI = "openai"
    ANTHROPIC = "anthropic"
    ZHIPU = "zhipu"
    QWEN = "qwen"
...
class LLMProvider(BaseModel):
    """大模型厂家配置 — 对应 llm_providers 集合，存储厂商API地址、密钥等"""
...
class ModelInfo(BaseModel):
    """模型信息 — 单个 LLM 模型的名称、定价、上下文长度、能力标签"""
...
class ModelCatalog(BaseModel):
    """模型目录 — 对应 model_catalog 集合，按厂商分组的模型列表"""
    id: Optional[PyObjectId] = Field(default_factory=PyObjectId, alias="_id")
...
class DataSourceType(str, Enum):
    """数据源类型枚举 — AKShare/Tushare/BaoStock 等

    注意：这个枚举与 sinoquant.constants.DataSourceCode 保持同步
    添加新数据源时，请先在 sinoquant/constants/data_sources.py 中注册
    """
    # 缓存数据源
    MONGODB = "mongodb"
...
class DatabaseType(str, Enum):
    """数据库类型枚举 — MongoDB/MySQL/PostgreSQL/Redis/SQLite"""
    MONGODB = "mongodb"
    MYSQL = "mysql"
...
class LLMConfig(BaseModel):
    """大模型配置 — 对应 config/models.json 中单个模型的配置项(模型名/密钥/参数/能力分级)"""
...
class DataSourceConfig(BaseModel):
    """数据源配置 — AKShare/Tushare/BaoStock 等数据源的连接参数和优先级"""
...
class MarketCategory(BaseModel):
    """市场分类配置 — 对应 market_categories 集合，如 A股/港股/美股"""
...
class UsageRecord(BaseModel):
    """使用记录 — 对应 token_usage 集合，记录每次 LLM 调用的 token 用量和费用"""
...
class UsageStatistics(BaseModel):
    """使用统计 — 按厂商/模型/日期汇总的 token 用量和费用统计"""
...
class SystemConfig(BaseModel):
    """系统配置模型 — 对应 system_configs 集合，全局 LLM/数据源/数据库配置"""
    id: Optional[PyObjectId] = Field(default_factory=PyObjectId, alias="_id")
