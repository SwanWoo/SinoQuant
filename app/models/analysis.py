...
对应 MongoDB 集合: analysis_tasks, analysis_reports
"""
...
class AnalysisStatus(str, Enum):
    """分析任务状态枚举 — pending(待处理) → processing(分析中) → completed/failed/cancelled"""
    PENDING = "pending"
...
class BatchStatus(str, Enum):
    """批量分析批次状态枚举 — 支持部分成功的 partial_success 状态"""
    PENDING = "pending"
    PROCESSING = "processing"
    COMPLETED = "completed"
...
class AnalysisTask(BaseModel):
    """分析任务模型 — 对应 analysis_tasks 集合，记录单次股票分析的全生命周期"""
...
class AnalysisHistoryQuery(BaseModel):
    """分析历史查询参数 — 按状态、日期、股票代码等条件筛选历史分析任务"""
