"""
SinaQuant v0.0.0-preview FastAPI Backend
主应用程序入口

Copyright (c) 2025 SinoQuant Project. All rights reserved.
版权所有 (c) 2025 SinoQuant Project。保留所有权利。

This software is proprietary and confidential. Unauthorized copying, distribution,
or use of this software, via any medium, is strictly prohibited.
本软件为专有和机密软件。严禁通过任何媒介未经授权复制、分发或使用本软件。

For commercial licensing, please contact: contact@sinoquant.dev
商业许可咨询，请联系：contact@sinoquant.dev
"""
...
from app.routers import vendor_config as vendor_config_router
from app.routers import alpha as alpha_router
...
async def _print_config_summary(logger):
    ...
    try:
        logger.info("=" * 70)
        logger.info("📋 SinaQuant Configuration Summary")
        logger.info("=" * 70)
...
async def lifespan(app: FastAPI):
    ...
    #  配置桥接：将统一配置写入环境变量，供 SinaQuant 核心库使用
    try:
    ...
    logger.info("SinaQuant FastAPI backend started")
    ...
    try:
        ...
        await close_db()
        logger.info("SinaQuant FastAPI backend stopped")
...
app.include_router(alpha_router.router, prefix="/api", tags=["alpha"])
...
# 厂商配置管理（第三方 API Key、URL 配置）
app.include_router(vendor_config_router.router, prefix="/api", tags=["vendor-config"])
