...
async def process_task(task_id: str) -> None:
    ...
    try:
        ...
        market_type = params.get("market_type", "A股")
        ...
        # Build request model and call the async analysis service
        try:
            from app.models.analysis import SingleAnalysisRequest, AnalysisParameters
            from app.services.simple_analysis_service import get_simple_analysis_service

            params_model = AnalysisParameters(
                market_type=market_type,
                analysis_date=analysis_date,
                research_depth=research_depth,
                selected_analysts=analysts,
            )
            request = SingleAnalysisRequest(symbol=symbol, parameters=params_model)

            service = get_simple_analysis_service()
            analysis_result = await service.execute_analysis_background(task_id, user_id, request)
