...
from sinoquant.dataflows.config import get_config, set_data_dir, get_data_dir
...
def demo_set_custom_data_dir():
    ...
    # 设置自定义数据目录
    custom_data_dir = os.path.join(os.path.expanduser("~"), "Documents", "SinaQuant_Custom", "data")
...
def demo_environment_variable_override():
    ...
    # 模拟设置环境变量
    test_env_dir = os.path.join(os.path.expanduser("~"), "Documents", "SinaQuant_ENV", "data")
...
def demo_directory_auto_creation():
    ...
    # 设置一个新的数据目录
    test_dir = os.path.join(os.path.expanduser("~"), "Documents", "SinaQuant_AutoCreate", "data")
...
```python
from sinoquant.dataflows.config import set_data_dir
...
def main():
    """主演示函数"""
    logger.info(f"[bold blue]🎯 SinaQuant 数据目录配置演示[/bold blue]")
    logger.info(f"=")
