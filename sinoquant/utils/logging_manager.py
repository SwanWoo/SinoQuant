...
_bootstrap_logger = logging.getLogger("sinoquant.logging_manager")
...
class SinaQuantLogger:
    ...
    def _add_file_handler(self, logger: logging.Logger):
        ...
        log_dir = Path(self.config['handlers']['file']['directory'])
        log_file = log_dir / 'sinoquant.log'
    ...
    def _add_structured_handler(self, logger: logging.Logger):
        ...
        log_file = log_dir / 'sinoquant_structured.log'
...
# 全局日志管理器实例
_logger_manager: Optional[SinaQuantLogger] = None


def get_logger_manager() -> SinaQuantLogger:
    """获取全局日志管理器实例"""
    global _logger_manager
    if _logger_manager is None:
        _logger_manager = SinaQuantLogger()
...
def setup_logging(config: Optional[Dict[str, Any]] = None):
    """设置项目日志系统（便捷函数）"""
    global _logger_manager
    _logger_manager = SinaQuantLogger(config)
    return _logger_manager
