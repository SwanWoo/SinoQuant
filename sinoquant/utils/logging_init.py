...
from sinoquant.utils.logging_manager import setup_logging, get_logger
...
def init_logging(config_override: Optional[dict] = None) -> None:
    ...
    # 获取初始化日志器
    logger = get_logger('sinoquant.init')
    
    # 记录初始化信息
    logger.info("🚀 SinaQuant 日志系统初始化完成")
...
def log_startup_info():
    """记录应用启动信息"""
    logger = get_logger('sinoquant.startup')
    
    logger.info("=" * 60)
    logger.info("🎯 SinaQuant 启动")
    logger.info("=" * 60)
...
def log_shutdown_info():
    """记录应用关闭信息"""
    logger = get_logger('sinoquant.shutdown')
    
    logger.info("=" * 60)
    logger.info("🛑 SinaQuant 关闭")
    logger.info("=" * 60)
