...
def create_bull_researcher(llm, memory, max_debate_rounds=1):
    def bull_node(state) -> dict:
        ...
        # 安全护栏：如果已超过最大发言次数，直接跳过
        current_count = investment_debate_state.get("count", 0)
        max_utterances = 2 * max_debate_rounds
        if current_count >= max_utterances:
            logger.warning(f"🐂 [护栏] 发言次数已达上限 ({current_count} >= {max_utterances})，跳过多头研究员")
...
⚠️ 重要提醒：当前分析的是中国A股，所有价格和估值请使用 {currency}（{currency_symbol}）作为单位。
