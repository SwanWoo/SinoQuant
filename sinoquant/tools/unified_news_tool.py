...
"""
统一新闻分析工具
整合A股市场的新闻获取逻辑到一个工具函数中
让大模型只需要调用一个工具就能获取A股新闻数据
"""
...
class UnifiedNewsAnalyzer:
    ...
    def _get_news_from_database(self, stock_code: str, max_news: int = 10) -> str:
        ...
        """
        try:
            from sinoquant.dataflows.cache.app_adapter import get_mongodb_client
    ...
    def _get_a_share_news(self, stock_code: str, max_news: int, model_info: str = "") -> str:
        ...
        # 本地数据模式：数据库无新闻时禁止调外部API
        try:
            from sinoquant.config.runtime_settings import local_data_only_enabled
            if local_data_only_enabled():
                logger.warning(
                    f"[统一新闻工具] 🔒 本地数据模式：数据库无新闻，跳过所有外部API"
                )
                return (
                    f"❌ [本地数据模式] MongoDB中没有{stock_code}的新闻数据。"
                    f"请先使用数据同步功能下载新闻数据，或关闭TA_LOCAL_DATA_ONLY模式。"
                )
        except Exception:
            pass
...
def create_unified_news_tool(toolkit):
    ...
    def get_stock_news_unified(stock_code: str, max_news: int = 100, model_info: str = ""):
        ...
        Args:
            stock_code (str): 股票代码 (支持A股如000001)
