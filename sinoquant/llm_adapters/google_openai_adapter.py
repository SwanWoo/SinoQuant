...
为 SinaQuant 提供Google AI (Gemini)模型的 OpenAI 兼容接口
...
class ChatGoogleOpenAI(ChatGoogleGenerativeAI):
    ...
    def __init__(self, base_url: Optional[str] = None, **kwargs):
        ...
        # 🔒 API Key 只接受通过 kwargs 传入（来自数据库/Web 界面配置），不从环境变量读取
        google_api_key = kwargs.get("google_api_key")

        if not google_api_key:
            logger.error("❌ [Google初始化] API Key 未传入")
            raise ValueError(
                "Google API密钥未找到。请在 Web 界面配置 API Key（设置 -> 大模型厂家）。"
            )
        else:
            logger.info("✅ [Google初始化] 使用传入的 API Key（来自数据库配置）")
