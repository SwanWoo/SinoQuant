...
class AKShareProvider(BaseStockDataProvider):
    ...
    async def get_historical_data(
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"⚠️ 东方财富stock_zh_a_hist失败: {e}, 尝试新浪备用源")

            # 方法2: 新浪财经备用源
            if hist_df is None or (hasattr(hist_df, 'empty') and hist_df.empty):
                hist_df = await self._fetch_historical_data_sina(code, start_date, end_date, period)
    ...
    async def _fetch_historical_data_sina(
        self, code: str, start_date: str, end_date: str, period: str = "daily"
    ) -> Optional[pd.DataFrame]:
        """新浪财经备用源获取历史K线数据（东方财富API被封时降级使用）"""
        try:
            period_map = {"daily": 240, "weekly": 1200, "monthly": 5200}
            ...
            hist_df = await asyncio.to_thread(fetch_sina)
            if hist_df is not None and not hist_df.empty:
                logger.info(f"✅ {code}新浪备用源获取成功: {len(hist_df)}条记录")
            return hist_df
        except Exception as e:
            logger.warning(f"⚠️ {code}新浪备用源也失败: {e}")
            return None
        ...
        try:
            ...
            if symbol:
                ...
                try:
                    ...
                    if news_df is not None and not news_df.empty:
                        ...
                        for _, row in news_df.head(limit).iterrows():
                            ...
                            news_item = {
                                ...
                                "url": url,
                                "source": "东方财富",
                                "publish_time": self._parse_news_time(news_time),
                                "category": self._classify_news(content, title),
                                "sentiment": self._analyze_news_sentiment(content, title),
                                "sentiment_score": self._calculate_sentiment_score(content, title),
                                "keywords": self._extract_keywords(content, title),
                    ...
                    self.logger.warning(f"东方财富全球快讯获取失败: {e}")
