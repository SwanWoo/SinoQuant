...
import logging
_logger = logging.getLogger("sinoquant.config")
...
def local_data_only_enabled(default: bool = False) -> bool:
    """是否启用纯本地数据模式（禁止外部API降级）。ENV: TA_LOCAL_DATA_ONLY; DB: ta_local_data_only

    当启用时，MongoDB未命中不会降级到外部API，而是返回明确的"数据不可用"消息。
    要求同时启用 TA_USE_APP_CACHE。
    """
    src = "default"
    env_val = os.getenv("TA_LOCAL_DATA_ONLY")
    try:
        eff = _get_system_settings_sync()
    except Exception:
        eff = {}
    if isinstance(eff, dict) and "ta_local_data_only" in eff:
        src = "db"
    elif env_val is not None and str(env_val).strip() != "":
        src = "env"

    val = get_bool("TA_LOCAL_DATA_ONLY", "ta_local_data_only", default)

    try:
        _logger.info(f"[runtime_settings] TA_LOCAL_DATA_ONLY evaluated -> {val} (source={src}, env={env_val})")
    except Exception:
        pass
    return val
