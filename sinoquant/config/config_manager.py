...
# 运行时设置：读取系统时区
from sinoquant.config.runtime_settings import get_timezone_name
...
class ConfigManager:
    ...
    def _init_mongodb_storage(self):
        ...
        try:
            ...
            database_name = os.getenv("MONGODB_DATABASE_NAME", "sinoquant")
    ...
    def _init_default_configs(self):
        ...
        if not self.settings_file.exists():
            ...
            default_settings = {
                ...
                "results_dir": os.path.join(os.path.expanduser("~"), "Documents", "SinaQuant", "results"),  # 结果目录
        ...
        try:
            if self.settings_file.exists():
                ...
                settings = {
                    ...
                    "cache_dir": os.path.join(os.path.expanduser("~"), "Documents", "SinaQuant", "data", "cache"),
                    "results_dir": os.path.join(os.path.expanduser("~"), "Documents", "SinaQuant", "results"),
...
def _get_project_config_dir():
    """获取项目根目录的配置目录"""
    # 从当前文件位置推断项目根目录
    current_file = Path(__file__)  # sinoquant/config/config_manager.py
