                ...
                <div v-if="analysisResults.decision" class="decision-section">
                  ...
                  <div class="decision-card">
                    ...
                    <div class="decision-reasoning">
                      <h5>分析依据:</h5>
                      <p>{{ removeThinkingContent(analysisResults.decision.reasoning) }}</p>
                ...
                <div v-if="analysisResults" class="overview-section">
                  ...
                  <div class="overview-card">
                    ...
                    <div v-if="analysisResults.summary" class="overview-summary">
                      <h5>分析摘要:</h5>
                      <p>{{ removeThinkingContent(analysisResults.summary) }}</p>
                    </div>

                    <div v-if="analysisResults.recommendation" class="overview-recommendation">
                      <h5>投资建议:</h5>
                      <p>{{ removeThinkingContent(analysisResults.recommendation) }}</p>
                    </div>
                  </div>
                </div>
...
// 模型设置 — 初始为空，由 initializeModelSettings 从系统配置加载
...
const onStockCodeInput = () => {
  ...
  // 显示格式提示
  stockCodeHelp.value = getStockCodeFormatHelp()
}
...
const validateStockCodeInput = () => {
  ...
  if (!validation.valid) {
    ...
    stockCodeError.value = ''
    stockCodeHelp.value = 'A stock code format is correct'
...
const getAnalysisReports = (data: any) => {
  ...
  // 分级精简展示策略：按深度展示不同层级的报告摘要
  const normalizeDepthLevel = (depth: any): number => {
  ...
  const analystAliasMap: Record<string, string> = {
    market: 'market',
  ...
  const analystReportKeyMap: Record<string, string> = {
  ...
  const depthLevel = normalizeDepthLevel(data?.research_depth)
  const selectedAnalysts = Array.isArray(data?.analysts) ? data.analysts : []
  let allowedReportKeys: Set<string> | null = new Set(['final_trade_decision'])

  // 5级（全面）不过滤，展示全量报告
  if (depthLevel >= 5) {
    allowedReportKeys = null
  }

  if (allowedReportKeys) {
    selectedAnalysts.forEach((analyst: string) => {
      const normalized = analystAliasMap[analyst] || analyst
      const reportKey = analystReportKeyMap[normalized]
      if (reportKey) {
        allowedReportKeys?.add(reportKey)
      }
    })
  }

  if (allowedReportKeys && depthLevel >= 2) {
    allowedReportKeys.add('research_team_decision')
  }
  if (allowedReportKeys && depthLevel >= 3) {
    allowedReportKeys.add('trader_investment_plan')
  }
  if (allowedReportKeys && depthLevel >= 4) {
    allowedReportKeys.add('risk_management_decision')
  }
  if (allowedReportKeys && depthLevel >= 5) {
    allowedReportKeys.add('investment_plan')
  }

  if (allowedReportKeys) {
    console.log('📉 [分级精简] 启用报告过滤，保留报告键:', Array.from(allowedReportKeys), 'depth=', depthLevel)
  } else {
    console.log('📈 [全面分析] 关闭报告过滤，展示全量报告', 'depth=', depthLevel)
  }
  ...
  reportMappings.forEach(mapping => {
    if (allowedReportKeys && !allowedReportKeys.has(mapping.key)) {
      return
    }
...
const removeThinkingContent = (text: string): string => {
  ...
  // 某些模型仅输出 </think> 结束标签，优先保留其后内容
  const closingThinkTagRe = /<\s*\/\s*(think|thinking|reasoning)\s*>/gi
  const closingTagMatches = Array.from(cleaned.matchAll(closingThinkTagRe))
  if (closingTagMatches.length > 0) {
    const lastMatch = closingTagMatches[closingTagMatches.length - 1]
    const tagText = lastMatch[0]
    const tagIndex = lastMatch.index ?? -1
    if (tagIndex >= 0) {
      const tail = cleaned.slice(tagIndex + tagText.length).trim()
      cleaned = tail || cleaned.slice(0, tagIndex)
    }
  }

  // 移除常见思维标签和思维代码块
  cleaned = cleaned
    .replace(/<\s*(think|thinking|reasoning)\b[^>]*>[\s\S]*?<\s*\/\s*\1\s*>/gi, '')
    .replace(/<\s*(think|thinking|reasoning)\b[^>]*>[\s\S]*$/gi, '')
  ...
  finalMarkers.forEach((marker) => {
    ...
    if (pos !== -1 && pos + marker.length > startPos) {
      startPos = pos + marker.length
    }
  })
  if (startPos !== -1) {
    cleaned = cleaned.slice(startPos)
  }

  const rolePrefix = '(?:[A-Za-z\\u4e00-\\u9fff][A-Za-z0-9_\\-/\\u4e00-\\u9fff ]{0,40}\\s*[:：-]\\s*)?'
  const reasoningKeywordCore = [
    'thinking process', 'reasoning process', 'chain of thought',
    'analysis of the request', 'analyze the request',
    'wait,\\s*i need to check', 'i need to check', 'let me think',
    'drafting strategy', 'content generation', 'review against constraints', 'final polish',
    'self-correction', 'user instruction block', 'tool output section',
    'analysis plan', 'constraint check', 'final output generation', 'final output construction',
    '思考过程', '思维过程', '推理过程', '让我先想', '我需要检查', '先检查一下'
  ].join('|')
  const reasoningLineRe = new RegExp(
    `^\\s*(?:[-*]|\\d+[.)])?\\s*(?:\\*\\*)?\\s*${rolePrefix}(?:${reasoningKeywordCore})(?:\\*\\*)?\\s*[:：-]?\\s*$`,
    'i'
  )
  const reasoningInlineRe = new RegExp(
    `\\b(?:${reasoningKeywordCore}|final check)\\b`,
    'i'
  )
  const reasoningMetaBulletRe = /^\s*(?:[-*]|\d+[.)])\s*(?:\*\*)?(context|correction|decision|plan|priority|instruction|prompt)\s*[:：]/i
  const reasoningShortLineRe = /^\s*(?:[-*]\s*)?(let['’]?s write\.?|let['’]?s draft\.?|let['’]?s go\.?|then it says|very complex prompt|this looks solid\.?|i will use this version\.?)\s*$/i
  const reasoningBlockStartRe = new RegExp(
    `^\\s*(?:[-*]|\\d+[.)])?\\s*(?:\\*\\*)?${rolePrefix}(?:thinking process|reasoning process|analysis of the request|analyze the request|wait,\\s*i need to check|drafting strategy|content generation|review against constraints|final polish|analysis plan|constraint check|final output generation|final output construction|思考过程|思维过程|推理过程|我需要检查|先检查一下)`,
    'i'
  )
  const planningLineRe = /^(?:\s*)(structure|content|tone|length|constraint check|analysis plan|review against constraints|final output generation|final output construction)\s*[:：]/i
  const finalMarkerLineRe = /^\s*(?:[-*]|\d+[.)])?\s*(?:\*\*)?(final answer|final output|final output generation|final output construction|最终答案|最终输出)(?:\*\*)?\s*[:：.]?\s*(?:\(.*\))?\s*$/i
  const contentHeadingRe = /^\s*(#{1,6}\s+|[一二三四五六七八九十]+[、.]\s*|\d+[、.]\s*|标题[:：]\s*|正文[:：]\s*|title[:：]\s*|body[:：]\s*|最终交易建议|投资建议|结论|摘要)/i
  const blockExitHeadingRe = /^\s*(#{1,6}\s+|[一二三四五六七八九十]+[、.]\s*|标题[:：]\s*|正文[:：]\s*|title[:：]\s*|body[:：]\s*|最终交易建议|投资建议|结论|摘要)/i

  // 行级过滤，清掉“Wait, I need to check ...”等思维草稿
  ...
  lines.forEach((line) => {
    ...
    if (!stripped) {
      if (!inReasoningBlock) filteredLines.push('')
      return
    }

    if (inReasoningBlock) {
      if (finalMarkerLineRe.test(stripped)) {
        inReasoningBlock = false
        return
      }

      if (blockExitHeadingRe.test(stripped)) {
        inReasoningBlock = false
        filteredLines.push(line)
        return
      }

      if (stripped.includes('报告') && stripped.length <= 80 && !reasoningInlineRe.test(stripped)) {
        inReasoningBlock = false
        filteredLines.push(line)
        return
      }

      if (reasoningLineRe.test(stripped)) return
      if (reasoningInlineRe.test(stripped)) return
      if (reasoningShortLineRe.test(stripped)) return
      if (planningLineRe.test(stripped)) return
      if (reasoningMetaBulletRe.test(stripped)) return
      if (/^(\*|-|\d+[.)])\s*/.test(stripped)) return

      const looksLikeContentParagraph =
        stripped.length >= 20 &&
        /[\u4e00-\u9fff]/.test(stripped) &&
        !reasoningInlineRe.test(stripped) &&
        !planningLineRe.test(stripped)

      if (looksLikeContentParagraph) {
        inReasoningBlock = false
        filteredLines.push(line)
      }
      return
    ...
    if (reasoningBlockStartRe.test(stripped)) {
      inReasoningBlock = true
      return
    }

    if (reasoningLineRe.test(stripped)) {
      inReasoningBlock = true
      return
    }
    ...
    if (reasoningShortLineRe.test(stripped)) {
      return
    }
    if (planningLineRe.test(stripped)) return

    const isBulletLike = /^(\*|-|\d+[.)])\s*/.test(stripped)
    if (isBulletLike && reasoningInlineRe.test(stripped)) {
      return
    }
    if (reasoningMetaBulletRe.test(stripped)) {
      return
    }
  ...
  // 若在 Markdown 正文标题前仍有元提示词前缀，直接从首个标题开始保留
  const headingMatch = cleaned.match(/^\s*#{1,6}\s+/m)
  if (headingMatch && headingMatch.index !== undefined && headingMatch.index > 0) {
    const prefix = cleaned.slice(0, headingMatch.index).trim()
    const metaPrefixRe = /(instruction|prompt|there is a section|this suggests|please write all analysis|priority:|你是一位|请使用中文撰写|以下是综合分析报告)/i
    if (prefix && (reasoningLineRe.test(prefix) || metaPrefixRe.test(prefix))) {
      cleaned = cleaned.slice(headingMatch.index).trim()
      cleaned = cleaned
        .split('\n')
        .filter((line) => !reasoningLineRe.test(line))
        .filter((line) => !reasoningShortLineRe.test(line))
        .filter((line) => !reasoningMetaBulletRe.test(line))
        .join('\n')
        .replace(/\n{3,}/g, '\n\n')
        .trim()
    }
  ...
  // 兜底：仍包含明显思维链痕迹时，优先保留从正文标题开始的部分
  if (reasoningLineRe.test(cleaned)) {
    const fallbackLines = cleaned.split('\n')
    const firstHeadingIndex = fallbackLines.findIndex((line) => {
      const s = line.trim()
      return contentHeadingRe.test(s) || (s.includes('报告') && s.length <= 80)
    })

    if (firstHeadingIndex !== -1) {
      cleaned = fallbackLines.slice(firstHeadingIndex).join('\n')
        .split('\n')
        .filter((line) => !reasoningLineRe.test(line))
        .filter((line) => !reasoningShortLineRe.test(line))
        .filter((line) => !reasoningMetaBulletRe.test(line))
...
const formatReportContent = (content: any) => {
  ...
  // 过滤思维链内容
  stringContent = removeThinkingContent(stringContent)
  console.log('✅ [DEBUG] 过滤思维链后长度:', stringContent.length)
    ...
    const MessageComponent = {
      setup() {
        ...
        return () => h('div', { style: 'line-height: 2;' }, [
          ...
          h('p', [
            h('strong', '预计金额：'),
            h('span', { style: 'color: #42b983; font-weight: bold;' }, `${estimatedAmount.value}元`)
          ]),
          h('p', [
            h('strong', '置信度：'),
...
const initializeModelSettings = async () => {
  try {
    ...
    // 获取模型列表，仅保留已启用且有 API Key 的
    const llmResponse: any = await configApi.getLLMConfigs()
    const llmConfigs = Array.isArray(llmResponse)
      ? llmResponse
      : (Array.isArray(llmResponse?.data) ? llmResponse.data : [])

    availableModels.value = llmConfigs.filter((m: any) => {
      const hasKey = m.api_key && m.api_key.length > 0 && !m.api_key.startsWith('your_')
      const isEnabled = m.enabled !== false
      return hasKey && isEnabled
    })

    // 仅当系统已配置默认模型时才自动填充；否则留空让用户手动选择
    if (modelSettings.value.quickAnalysisModel && availableModels.value.length > 0) {
      const availableModelNames = new Set(availableModels.value.map((m: any) => m.model_name))
      if (!availableModelNames.has(modelSettings.value.quickAnalysisModel)) {
        modelSettings.value.quickAnalysisModel = ''
      }
    }
    if (modelSettings.value.deepAnalysisModel && availableModels.value.length > 0) {
      const availableModelNames = new Set(availableModels.value.map((m: any) => m.model_name))
      if (!availableModelNames.has(modelSettings.value.deepAnalysisModel)) {
        ...
        .config-section {
          ...
          .action-buttons {
            ...
            .submit-btn.el-button {
              ...
              background: linear-gradient(135deg, #42b983 0%, #369a6d 100%) !important;
              border: none !important;
              border-radius: 16px !important;
              transition: all 0.3s ease !important;
              box-shadow: 0 4px 15px rgba(66, 185, 131, 0.2) !important;
              min-width: 280px !important;
              max-width: 280px !important;

              &:hover {
                transform: translateY(-3px) !important;
                box-shadow: 0 12px 30px rgba(66, 185, 131, 0.4) !important;
                background: linear-gradient(135deg, #42b983 0%, #369a6d 100%) !important;
              }
              ...
              &:disabled {
                ...
                box-shadow: 0 4px 15px rgba(66, 185, 131, 0.1) !important;
              }
...
.step-item {
  ...
  &.step-current {
    ...
    background: linear-gradient(90deg, rgba(66, 185, 131, 0.05) 0%, transparent 100%);

    .step-icon {
      background: linear-gradient(135deg, #42b983 0%, #369a6d 100%);
      color: white;
      box-shadow: 0 2px 12px rgba(66, 185, 131, 0.4);
    }
...
.main-progress-bar {
  ...
  :deep(.el-progress-bar__inner) {
    background: linear-gradient(90deg, #42b983 0%, #369a6d 100%);
    border-radius: 8px;
