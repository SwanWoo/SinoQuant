    ...
    const MessageComponent = {
      setup() {
        ...
        return () => h('div', { style: 'line-height: 2;' }, [
          // 风险提示横幅
          h('div', {
            style: 'background-color: var(--el-color-danger-light, rgba(245,108,108,0.1)); border: 1px solid var(--el-color-danger); border-radius: 4px; padding: 12px; margin-bottom: 16px;'
          }, [
            ...
            h('div', { style: 'color: var(--el-text-color-secondary); font-size: 12px; line-height: 1.6;' }, [
...
const generateStrategy = async () => {
  ...
  try {
    const res = await alphaApi.generateStrategy({
      symbol: report.value.stock_symbol,
      analysis_task_id: report.value.task_id || '',
    })
    if (res.success) {
...
const getConfidenceColor = (score: number) => {
  ...
  if (score >= 60) return '#42b983' // 中上 - 蓝色
