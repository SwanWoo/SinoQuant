...
def setup_cli_logging():
    ...
    """
    import logging
    from sinoquant.utils.logging_manager import get_logger_manager
    ...
    # 同时移除sinoquant日志器的控制台处理器
    sinoquant_logger = logging.getLogger('sinoquant')
    for handler in sinoquant_logger.handlers[:]:
        if isinstance(handler, logging.StreamHandler) and hasattr(handler, 'stream'):
            if handler.stream.name in ['<stderr>', '<stdout>']:
                sinoquant_logger.removeHandler(handler)
...
app = typer.Typer(
    name="SinaQuant",
    help="SinaQuant CLI: 多智能体大语言模型金融交易框架 | Multi-Agents LLM Financial Trading Framework",
...
def update_display(layout, spinner_text=None):
    ...
    layout["header"].update(
        Panel(
            "[bold green]Welcome to SinaQuant CLI[/bold green]\n"
            "[dim]© [项目团队](https://github.com/SinoQuant)[/dim]",
            title="Welcome to SinaQuant",
            border_style="green",
            padding=(1, 2),
            expand=True,
        )
...
def get_user_selections():
    ...
    try:
        ...
        welcome_ascii = "SinaQuant"

    # Create welcome box content
    welcome_content = f"{welcome_ascii}\n"
    welcome_content += "[bold green]SinaQuant: 多智能体大语言模型金融交易框架 - CLI[/bold green]\n"
    ...
    welcome_content += (
        "[dim]Built by [项目团队](https://github.com/SinoQuant)[/dim]"
    )
    ...
    welcome_box = Panel(
        ...
        border_style="green",
        padding=(1, 2),
        title="欢迎使用 SinaQuant | Welcome to SinaQuant",
...
def get_ticker(market):
    ...
    while True:
        ...
        if re.match(market['pattern'], ticker):
...
def display_complete_report(final_state):
    ...
    # 辅助函数：安全获取并清理报告内容
    def get_clean_report(state, key):
        content = state.get(key)
        if isinstance(content, str):
            return remove_thinking_content(content)
        return content
    ...
    market_report = get_clean_report(final_state, "market_report")
    if market_report:
    ...
    sentiment_report = get_clean_report(final_state, "sentiment_report")
    if sentiment_report:
        analyst_reports.append(
            Panel(
                Markdown(sentiment_report),
    ...
    # News Analyst Report
    news_report = get_clean_report(final_state, "news_report")
    if news_report:
        analyst_reports.append(
            Panel(
                Markdown(news_report),
                title="News Analyst",
                border_style="blue",
                padding=(1, 2),
            )
    ...
    fundamentals_report = get_clean_report(final_state, "fundamentals_report")
    if fundamentals_report:
        analyst_reports.append(
            Panel(
                Markdown(fundamentals_report),
    ...
    investment_debate = get_clean_report(final_state, "investment_debate_state")
    if investment_debate and isinstance(investment_debate, dict):
        research_reports = []
        debate_state = investment_debate

        # Bull Researcher Analysis
        bull_history = debate_state.get("bull_history")
        if bull_history and isinstance(bull_history, str):
            bull_history = remove_thinking_content(bull_history)
            research_reports.append(
                Panel(
                    Markdown(bull_history),
        ...
        if bear_history and isinstance(bear_history, str):
            bear_history = remove_thinking_content(bear_history)
            research_reports.append(
                Panel(
                    Markdown(bear_history),
        ...
        if judge_decision and isinstance(judge_decision, str):
            judge_decision = remove_thinking_content(judge_decision)
            research_reports.append(
                Panel(
    ...
    trader_plan = get_clean_report(final_state, "trader_investment_plan")
    if trader_plan:
        console.print(
            Panel(
                Panel(
                    Markdown(trader_plan),
                    title="Trader",
                    border_style="blue",
                    padding=(1, 2),
                ),
    ...
    risk_debate = get_clean_report(final_state, "risk_debate_state")
    if risk_debate and isinstance(risk_debate, dict):
        risk_reports = []
        risk_state = risk_debate

        # Aggressive (Risky) Analyst Analysis
        risky_history = risk_state.get("risky_history")
        if risky_history and isinstance(risky_history, str):
            risky_history = remove_thinking_content(risky_history)
            risk_reports.append(
                Panel(
                    Markdown(risky_history),
        ...
        if safe_history and isinstance(safe_history, str):
            safe_history = remove_thinking_content(safe_history)
            risk_reports.append(
                Panel(
                    Markdown(safe_history),
        ...
        if neutral_history and isinstance(neutral_history, str):
            neutral_history = remove_thinking_content(neutral_history)
            risk_reports.append(
                Panel(
                    Markdown(neutral_history),
        ...
        if risk_judge and isinstance(risk_judge, str):
            risk_judge = remove_thinking_content(risk_judge)
            console.print(
                Panel(
                    Panel(
                        Markdown(risk_judge),
...
def check_api_keys(llm_provider: str) -> bool:
    ...
    # 检查金融数据API密钥（A股使用tushare/akshare/baostock，无需finnhub）
    ...
    if missing_keys:
        ...
        logger.info(f"   TUSHARE_TOKEN=your_tushare_token")
...
def run_analysis():
    ...
    with Live(layout, refresh_per_second=DEFAULT_REFRESH_RATE) as live:
        ...
        final_state = trace[-1]
        # 优先使用 Trader 节点直接输出的结构化决策，回退到 process_signal
        decision = final_state.get("structured_decision")
        if not decision or not isinstance(decision, dict):
            decision = graph.process_signal(final_state.get("final_trade_decision", ""), selections['ticker'])
...
def config():
    ...
    """
    logger.info(f"\n[bold blue]🔧 SinaQuant 配置 | Configuration[/bold blue]")
    ...
    api_keys_table.add_row(
        "TUSHARE_TOKEN",
        "✅ 已配置" if tushare_token else "❌ 未配置",
        f"A股数据 | {tushare_token[:DEFAULT_API_KEY_DISPLAY_LENGTH]}..." if tushare_token else "A股数据API Token"
    )
    api_keys_table.add_row(
        "OPENAI_API_KEY",
    ...
    logger.info(f"   - TUSHARE_TOKEN (A股数据 | A-share data)")
...
def version():
    ...
    logger.info(f"\n[bold blue]📊 SinaQuant 版本信息 | Version Information[/bold blue]")
    ...
    logger.info(f"[green]基于项目 | Based on:[/green] [blue]SinoQuant/SinaQuant[/blue]")
    logger.info(f"[green]创建目的 | Purpose:[/green] [cyan]更好地在中国推广SinaQuant[/cyan]")
    ...
    logger.info(f"  • 💎 感谢 项目团队 团队提供的珍贵源码")
    logger.info(f"  • 🔄 感谢持续的维护、更新和改进工作")
    logger.info(f"  • 🌍 感谢选择Apache 2.0协议的开源精神")
    logger.info(f"  • 🎯 本项目旨在更好地在中国推广SinaQuant")
    logger.info(f"  • 🔗 源项目: https://github.com/SinoQuant/SinaQuant")
...
def data_config(
    ...
    if reset:
        # 重置为默认配置
        default_data_dir = os.path.join(os.path.expanduser("~"), "Documents", "SinaQuant", "data")
    ...
    logger.info(f"• 设置数据目录: sinoquant data-config --set /path/to/data")
    logger.info(f"• 重置为默认: sinoquant data-config --reset")
    logger.info(f"• 查看当前配置: sinoquant data-config --show")
...
def examples():
    ...
    """
    logger.info(f"\n[bold blue]📚 SinaQuant 示例程序 | Example Programs[/bold blue]")
...
def test():
    ...
    Run system tests
    """
    logger.info(f"\n[bold blue]🧪 SinaQuant 测试 | Tests[/bold blue]")
...
def help_chinese():
    ...
    """
    logger.info(f"\n[bold blue]📖 SinaQuant 中文帮助 | Chinese Help[/bold blue]")
    ...
    logger.info(f"• GitHub: https://github.com/SinoQuant/SinaQuant")
