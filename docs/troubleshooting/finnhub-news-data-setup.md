...
获取新闻数据失败: [Errno 2] No such file or directory: '/path/to/data\\finnhub_data\\news_data\\AAPL_data_formatted.json'
```
...
我们已经修复了 `sinoquant/default_config.py` 中的路径配置：
...
# 修复后（跨平台兼容路径）
"data_dir": os.path.join(os.path.expanduser("~"), "Documents", "SinaQuant", "data"),
```
...
# 测试新闻数据获取
python -c "
from sinoquant.dataflows.interface import get_finnhub_news
...
```python
from sinoquant.dataflows.config import set_config
...
```bash
# Windows
set SINOQUANT_DATA_DIR=C:\your\custom\data\directory

# Linux/Mac
export SINOQUANT_DATA_DIR=/your/custom/data/directory
```
...
```bash
# Windows（以管理员身份运行）
mkdir "C:\Users\%USERNAME%\Documents\SinaQuant\data"

# Linux/Mac
mkdir -p ~/Documents/SinaQuant/data
chmod 755 ~/Documents/SinaQuant/data
```
...
1. **`sinoquant/default_config.py`**
   - 修复硬编码的Unix路径
   - 使用跨平台兼容的路径构建

2. **`sinoquant/dataflows/finnhub_utils.py`**
   - 添加文件存在性检查
   - 改进错误处理和调试信息
   - 使用UTF-8编码读取文件

3. **`sinoquant/dataflows/interface.py`**
