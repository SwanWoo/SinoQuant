...
本指南详细说明如何在SinaQuant系统中接入新的股票数据SDK，包括架构设计、接入流程、代码规范和测试验证。
...
```
外部SDK → sinoquant适配器 → app数据同步服务 → MongoDB存储 → app查询服务 → API/前端
```
...
### 步骤1: 创建SDK适配器 (sinoquant层)

在 `sinoquant/dataflows/` 目录下创建新的SDK适配器：

```python
# sinoquant/dataflows/new_sdk_provider.py
...
from sinoquant.dataflows.new_sdk_provider import NewSDKProvider

class NewSDKSyncService:
    """新SDK数据同步服务 - 负责数据库操作和业务逻辑"""

    def __init__(self):
        self.provider = NewSDKProvider()  # 使用sinoquant的适配器
        self.stock_service = get_stock_data_service()  # 使用app的数据服务

    async def sync_basic_info(self):
        """同步基础信息到数据库"""
        # 1. 从sinoquant适配器获取标准化数据
...
```python
from sinoquant.config.runtime_settings import get_setting
...
```bash
# 复制示例适配器 (sinoquant层 - 纯数据获取)
cp sinoquant/dataflows/example_sdk_provider.py sinoquant/dataflows/your_sdk_provider.py
...
```python
# 修改 sinoquant/dataflows/your_sdk_provider.py
...
python -c "
import asyncio
from sinoquant.dataflows.your_sdk_provider import YourSDKProvider
...
- [基础提供器接口](../../sinoquant/dataflows/base_provider.py)
- [示例SDK适配器](../../sinoquant/dataflows/example_sdk_provider.py)
