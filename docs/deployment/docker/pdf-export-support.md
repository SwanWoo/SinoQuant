...
SinaQuant 的 Docker 镜像已经内置了完整的 PDF 导出支持，包括：
...
```bash
# 拉取镜像
docker pull sinoquant/sinoquant-cn:latest
...
# 或手动构建
docker build -f Dockerfile.backend -t sinoquant-backend:latest .
```
...
docker run --rm -d \
    --name sinoquant-test \
    -p 8000:8000 \
    sinoquant-backend:latest
```

#### 2. 检查 WeasyPrint

```bash
docker exec sinoquant-test python -c "
import weasyprint
...
```bash
docker exec sinoquant-test pandoc --version
```

#### 5. 检查 wkhtmltopdf

```bash
docker exec sinoquant-test wkhtmltopdf --version
```

#### 6. 停止容器

```bash
docker stop sinoquant-test
```
...
4. **清理缓存重新构建**：
   ```bash
   docker build --no-cache -f Dockerfile.backend -t sinoquant-backend:latest .
   ```
...
```bash
# 启动容器
docker run --rm -it sinoquant-backend:latest bash
