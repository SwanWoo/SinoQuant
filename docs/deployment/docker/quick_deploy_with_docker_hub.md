# 🚀 SinaQuant 快速部署指南（Docker Hub 镜像）
...
如果确实想完全覆盖数据，可以使用：
```bash
docker exec -it sinoquant-backend python scripts/import_config_and_create_user.py --overwrite
```
...
docker exec sinoquant-mongodb mongodump \
  ...
  -d sinoquant -o /data/backup

# 复制备份到宿主机
docker cp sinoquant-mongodb:/data/backup ./mongodb_backup
```
