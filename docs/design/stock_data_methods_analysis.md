# SinaQuant 股票数据获取方法整理

## 📋 概述

本文档整理了 `D:\code\SinaQuant\sinoquant` 目录下所有股票数据获取相关的函数和方法，按照架构层次和数据类型进行分类。
...
#### 统一API (`sinoquant/api/stock_api.py`)
```python
...
#### 股票API (`sinoquant/dataflows/stock_api.py`)
```python
def get_stock_info(stock_code: str) -> Optional[Dict[str, Any]]
def get_stock_data(stock_code: str, start_date: str, end_date: str) -> str
```

#### 接口层 (`sinoquant/dataflows/interface.py`)
```python
# 中国股票数据
...
#### 中国股票数据提供器 (`sinoquant/dataflows/optimized_china_data.py`)
```python
...
#### 美股数据提供器 (`sinoquant/dataflows/optimized_us_data.py`)
```python
...
#### 港股数据工具 (`sinoquant/dataflows/hk_stock_utils.py`)
```python
...
#### Tushare适配器 (`sinoquant/dataflows/tushare_utils.py`)
```python
...
#### AKShare适配器 (`sinoquant/dataflows/akshare_utils.py`)
```python
...
#### Yahoo Finance适配器 (`sinoquant/dataflows/yfin_utils.py`)
```python
class YFinanceUtils:
    def get_stock_data(symbol: str, start_date: str, end_date: str, save_path: SavePathType = None) -> DataFrame
```

#### BaoStock适配器 (`sinoquant/dataflows/baostock_utils.py`)
```python
...
#### TDX适配器 (`sinoquant/dataflows/tdx_utils.py`)
```python
...
#### 股票数据服务 (`sinoquant/dataflows/stock_data_service.py`)
```python
...
#### 实时新闻工具 (`sinoquant/dataflows/realtime_news_utils.py`)
```python
...
```python
# 中国A股 - 推荐
from sinoquant.dataflows import get_china_stock_data_unified, get_china_stock_info_unified

# 美股 - 推荐  
from sinoquant.dataflows.optimized_us_data import get_us_stock_data_cached

# 港股 - 推荐
from sinoquant.dataflows.interface import get_hk_stock_data_unified
