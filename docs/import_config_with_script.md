...
```bash
# 使用 scp
scp database_export_config_2025-10-16.json user@new-server:/path/to/SinaQuant/
...
📝 后续步骤:
   1. 重启后端服务: docker restart sinoquant-backend
   2. 访问前端并使用默认账号登录
   3. 检查系统配置是否正确加载
```
...
用户名          邮箱                           角色       状态       创建时间
------------------------------------------------------------------------------------------
admin           admin@sinoquant.cn         管理员     激活       2025-10-16 10:30
...
```python
MONGO_URI = "mongodb://admin:sinoquant123@localhost:27017/sinoquant?authSource=admin"
DB_NAME = "sinoquant"
```
...
# 如果未运行，启动 MongoDB
docker start sinoquant-mongodb
```
...
docker exec sinoquant-mongodb mongodump \
  -u admin -p sinoquant123 --authenticationDatabase admin \
  -d sinoquant -o /tmp/backup

docker cp sinoquant-mongodb:/tmp/backup ./backup_before_import
```
