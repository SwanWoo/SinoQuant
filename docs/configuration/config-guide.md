...
SinaQuant 提供了统一的配置系统，所有配置通过 `.env` 文件管理。本指南详细介绍了所有可用的配置选项和最佳实践，包括v0.1.7新增的Docker部署和报告导出配置。
...
# 📁 路径配置
SINOQUANT_RESULTS_DIR=./results
SINOQUANT_DATA_DIR=./data
```
...
#### data_cache_dir
- **类型**: `str`
- **默认值**: `"sinoquant/dataflows/data_cache"`
- **说明**: 数据缓存目录
...
export SINOQUANT_RESULTS_DIR="/custom/results/path"
```
...
SINOQUANT_RESULTS_DIR=./custom_results
SINOQUANT_LOG_LEVEL=INFO
```
...
通过合理的配置，您可以根据不同的使用场景优化 SinaQuant 的性能和成本。
