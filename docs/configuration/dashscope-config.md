...
阿里百炼（DashScope）是阿里云推出的大模型服务平台，提供通义千问系列模型。本指南详细介绍如何在 SinaQuant 中配置和使用阿里百炼大模型。

## 🎉 v0.1.6 重大更新

### OpenAI兼容适配器
SinaQuant现在提供了全新的阿里百炼OpenAI兼容适配器，解决了之前的工具调用问题：
...
# 使用自定义参数创建LLM
from sinoquant.llm_adapters import ChatDashScope
...
#### 1. 自定义适配器
```python
from sinoquant.llm_adapters.openai_compatible_base import OpenAICompatibleBase
...
阿里百炼OpenAI兼容适配器的引入标志着SinaQuant在LLM集成方面的重大进步：
