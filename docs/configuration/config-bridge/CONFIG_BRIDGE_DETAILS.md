...
配置桥接模块 (`app/core/config_bridge.py`) 负责将统一配置系统中的配置桥接到环境变量，让 SinaQuant 核心库能够使用用户在 Web 界面中配置的参数。
...
os.environ['SINOQUANT_DEFAULT_MODEL'] = default_model

# 快速分析模型
quick_model = unified_config.get_quick_analysis_model()
os.environ['SINOQUANT_QUICK_MODEL'] = quick_model

# 深度分析模型
deep_model = unified_config.get_deep_analysis_model()
os.environ['SINOQUANT_DEEP_MODEL'] = deep_model
```
...
SinaQuant 的数据源配置管理器 (`sinoquant/config/providers_config.py`) 从环境变量读取：
...
SinaQuant 的运行时设置 (`sinoquant/config/runtime_settings.py`) 从环境变量读取：
